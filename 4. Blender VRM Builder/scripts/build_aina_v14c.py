...
"""
build_aina_v14c.py  –  Aina Venara VRM v14c
=============================================
Strategy:
  1. Clear scene.
  2. Import VRoid base (Aina_Venara_Base.vrm).
     Keep only the Face and Body skin. Delete default T-shirt, shorts, shoes.
  3. Append all high-quality custom hair, accessories, and outfit meshes
     from the master blend (Aina_Venara_Master.blend).
  4. Apply Subdivision Surface modifiers to the imported outfit/hair meshes
     to make them smooth, detailed, and organic (eliminating the blocky look).
  5. Weight-paint (data transfer) the outfit meshes from the base Body skin
     so they deform perfectly with the armature.
  6. Rig the hair and head accessories 100% to the Head bone.
  7. Apply correct material colors (cyan hair, blue-violet tips, teal eyes, pink glasses).
...
OUT  = os.path.join(ROOT, "output", "Aina_Venara_v14c.vrm")
PREV = os.path.join(ROOT, "output", "previews", "v14c")
...
# ── 1. Clear scene ────────────────────────────────────────────────────────
print("[v14c] Clearing scene...")
for obj in list(bpy.data.objects):
    bpy.data.objects.remove(obj, do_unlink=True)
purge()

# ── 2. Import VRoid base ──────────────────────────────────────────────────
print("[v14c] Importing VRoid base...")
...
# Get body mesh and delete T-shirt, shorts, sneakers vertices
# Body Material Slots:
...
if body:
    print("[v14c] Deleting clothes vertices from VRoid Body...")
    import bmesh
    me = body.data
    bm = bmesh.new()
    bm.from_mesh(me)
...
# ── 3. Append meshes from Master Blend ────────────────────────────────────
print("[v14c] Appending objects from Master Blend...")
with bpy.data.libraries.load(MASTER_BLEND) as (data_from, data_to):
    # Import all meshes and curves (except default Camera and Light)
    data_to.objects = [name for name in data_from.objects if name not in ["Camera", "Light", "QA_Area_Key_Light"]]
...
print("[v14c] Configuring materials and subdivisions...")
...
for obj in imported_objects:
    ...
    if obj.type == "MESH":
        ...
        # Exclude flat accessories that need sharp edges (like the hairclip-3)
...
for mat in bpy.data.materials:
    ...
    if any(x in mn for x in ["eye","iris"]):
        # Find color socket on BSDF or MToon
...
print("[v14c] Rigging imported meshes to VRoid Armature...")
for obj in imported_objects:
    if obj.type not in ["MESH", "CURVE"]:
        continue
...
# Custom camera look points per view for perfect framing!
...
# Zoom directly onto face (around head_z + 0.05)
...
print("[v14c] All processes completed!")
